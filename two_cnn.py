...
class ResNet(nn.Module):
    ...
    def __init__(self, bottleneck, stages, image_channels, num_classes):
        ...
        #making pure CNN resnet
        self.input_channels = 64
        self.pure_conv1 = nn.Conv2d(image_channels, 64, kernel_size=7, stride=2, padding=3)
        self.pure_bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU()
        self.pure_maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.pure_stage1 = self._make_layer(bottleneck, stages[0], out_channels=64, stride=1)
        self.pure_stage2 = self._make_layer(bottleneck, stages[1], out_channels=128, stride=2)
        self.pure_stage3 = self._make_layer(bottleneck, stages[2], out_channels=256, stride=2)
        self.pure_stage4 = self._make_layer(bottleneck, stages[3], out_channels=512, stride=2)
        self.pure_avgpool = nn.AdaptiveAvgPool2d((1,1))
        self.pure_fc = nn.Linear(2048, num_classes)
    ...
    def _forward_impl(self,x,topo_feature):
        ...
        #pure CNN
        cnn_feat = x.clone()
        cnn_feat = self.pure_conv1(cnn_feat)
        cnn_feat = self.pure_bn1(cnn_feat)
        cnn_feat = self.relu(cnn_feat)
        cnn_feat = self.pure_maxpool(cnn_feat)
        cnn_feat = self.pure_stage1(cnn_feat)
        cnn_feat = self.pure_stage2(cnn_feat)
        cnn_feat = self.pure_stage3(cnn_feat)
        cnn_feat = self.pure_stage4(cnn_feat)
        cnn_feat = self.pure_avgpool(cnn_feat)
        cnn_feat = cnn_feat.reshape(cnn_feat.shape[0], -1)
        cnn_feat = self.pure_fc(cnn_feat)
