...
def train_model():
    ...
    criterion_3 = nn.CrossEntropyLoss().cuda()
    ...
    for epoch in range(epochs):
        for batch_idx, batch in enumerate(dataloader):
            ...
            loss3 = criterion_3(res_outputs,labels)
            loss = loss1 + vision_topo_ratio*loss2 +pure_cnn_ratio*loss3
            if torch.isnan(loss):
            ...
            accuracy = compute_accuracy(cv_outputs + vision_topo_ratio*topo_outputs + pure_cnn_ratio*loss3, labels)
            if (batch_idx + 1) % 10 == 0:
                print(f'Epoch [{epoch+1}/{epochs}], Step [{batch_idx+1}/{len(dataloader)}], Res Loss: {loss1.item():.4f},Topo Loss: {loss2.item():.4f},Purecnn Loss: {loss3.item():.4f}, Batch Acc: {accuracy:.2f}%')

        # Save checkpoint at the end of each epoch
        if epoch >(epochs-4):
            checkpoint_filename = f"twocnn_checkpoint_epoch_{epoch+1}.pth.tar"
