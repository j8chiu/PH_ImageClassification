...
class TopoViT(nn.Module):
    def __init__(self, model_name='B_16_imagenet1k', image_size=224, device='cuda'):
        super(TopoViT, self).__init__()
        self.vit = ViT(model_name, pretrained=True, image_size=image_size).to(device)
        self.pd_encoder = PersistenceDiagramEncoder(input_dim=4)
        #self.topo_attention_mod = nn.ModuleList([nn.Sequential(nn.Linear(2048, 256), nn.Tanh()) for _ in range(4)])
        self.cls_token = nn.Parameter(torch.zeros(1, 1, 768,dtype=torch.float32))
        self.cls_head = nn.Linear(768, 7)
        self.topo_mods = nn.ModuleList([
            nn.Linear(1024, 768) for _ in range(4)
        ])
    def forward(self, x, topo_feature):
        N,_,H,W = x.shape
        x = self.vit.patch_embedding(x)
        x = x.flatten(2).transpose(1, 2)
        x = torch.cat((self.cls_token.expand(N, -1, -1), x), dim=1) # b,num_patches+1,d
        x = x + self.vit.positional_embedding(x)
        ...
        for i, block in enumerate(self.vit.transformer.blocks):
            if i in [2, 5, 8, 11]:
                topo_mod = self.topo_mods[i // 3](topo_feature_vector)
                topo_mod = topo_mod.unsqueeze(1).expand(-1, x.size(1), -1)  # Expand to match x dimensions
                modulated_x = x * topo_mod  # Element-wise multiplication
                x = x + modulated_x  # Add modulated features back to the original features
            else:
                x = block(x,mask= None)
...
def load_model(args,num_classes=7):
    if args.model_name == 'swin':
        ...
        model = TopoViT('B_16_imagenet1k').to(args.device)
        ...
        for _, p in model.cls_head.named_parameters():
            p.requires_grad = True
        for _, p in model.pd_encoder.named_parameters():
            p.requires_grad = True
        for mod in model.topo_mods:
...
def main(args):
    ...
    dataset_paths = {'Prostate': os.path.join(home_dir, 'Prostate/rois'),
                     ...
                     "ISIC": os.path.join(home_dir, "ISIC")}
    ...
    train_loader = DataLoader(train_set, batch_size=args.batch_size,
                              shuffle=True,num_workers=12,collate_fn=collate_fn)
    val_loader = DataLoader(val_set, batch_size=args.batch_size,
                            shuffle=False,num_workers=12,collate_fn=collate_fn)
    ...
    for epoch in (range(args.epochs)):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            criterion = torch.nn.CrossEntropyLoss()
            target = target.argmax(dim=1) if target.ndim > 1 else target
            target = target.to(device)
            #print('target: ',target)
            loss = criterion(pred, target)
            #class_label = torch.argmax(target, dim=1)
