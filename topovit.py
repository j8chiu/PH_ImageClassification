...
class TopoViT(nn.Module):
    def __init__(self, model_name='B_16_imagenet1k', image_size=224, device='cpu'):
    ...
    def forward(self, x, topo_feature):
        ...
        for i, block in enumerate(self.vit.transformer.blocks):
            if i in [2, 5, 8, 11]:
                topo_mod = self.topo_attention_mod[i // 3](topo_feature_vector)
                attention_scores = block.attention.compute_attention_scores(x)
                modified_attention_scores = attention_scores + topo_mod 
                x = block.attention.apply_attention_scores(modified_attention_scores, x)
            else:
                x = block(x)
