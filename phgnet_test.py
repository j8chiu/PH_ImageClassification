...
def test_model():
    ...
    model.load_state_dict(torch.load('phg_net_newratio_checkpoint_epoch_100.pth.tar')['state_dict'])
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(test_loader):
            ...
            loss = criterion(cv_outputs + 0.08*topo_outputs, labels)
            total_loss += loss.item() * labels.size(0)
            accuracy = compute_accuracy(cv_outputs+ 0.08*topo_outputs, labels)
